...
/**
 * A lowest precedence {@link EnvironmentPostProcessor} implementation to append Gemini default {@link PropertySource}
 * with lowest order in {@link Environment}
 *
 * @author <a href="mailto:mercyblitz@gmail.com">Mercy</a>
 * @since 0.2.2
 */
public class GeminiDefaultPropertySourceEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {

    /**
     * The name of Nacos default {@link PropertySource}
     */
    public static final String PROPERTY_SOURCE_NAME = "gemini-default";
    ...
    public static final String RESOURCE_LOCATION_PATTERN = CLASSPATH_ALL_URL_PREFIX
        + "META-INF/gemini-default.properties";
    ...
    private void processPropertySource(ConfigurableEnvironment environment, ResourceLoader resourceLoader) {
        ...
        try {
            PropertySource geminiDefaultPropertySource = buildPropertySource(resourceLoader);
            MutablePropertySources propertySources = environment.getPropertySources();
            // append geminiDefaultPropertySource as last one in order to be overrided by higher order
            propertySources.addLast(geminiDefaultPropertySource);
        } catch (IOException e) {
