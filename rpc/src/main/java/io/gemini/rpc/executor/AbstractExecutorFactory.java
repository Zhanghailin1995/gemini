...
public abstract class AbstractExecutorFactory implements ExecutorFactory {
    ...
    protected ThreadFactory threadFactory(String name) {
        ...
        // use netty DefaultThreadFactory
    ...
    protected int coreWorkers(Target target) {
        switch (target) {
            case CONSUMER:
                return SystemPropertyUtil.getInt(CONSUMER_EXECUTOR_CORE_WORKERS, Constants.AVAILABLE_PROCESSORS << 1);
            case PROVIDER:
                return SystemPropertyUtil.getInt(PROVIDER_EXECUTOR_CORE_WORKERS, Constants.AVAILABLE_PROCESSORS << 1);
            default:
