...
import io.gemini.transport.netty.handler.ChannelHandlerHolder;
...
public abstract class ConnectionWatchdog extends ChannelInboundHandlerAdapter implements TimerTask, ChannelHandlerHolder {
    ...
    private final ChanGroup group;

    private volatile int state = ST_STARTED;
    private int attempts;

    public ConnectionWatchdog(Bootstrap bootstrap, Timer timer, SocketAddress remoteAddress, ChanGroup group) {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        if (group != null) {
            group.add(NettyChan.attachChannel(ch));
        }
